...
export default function EditPhotoPage() {
  ...
  const encodedImageUrl = params.url as string;
  ...
  useEffect(() => {
    const fetchPhoto = async () => {
      setIsLoading(true);
      try {
        const response = await fetch(`/api/admin/photo/${encodedImageUrl}`);
        ...
        // Set form data with the fetched photo details
        setFormData({
          title: photo.title || "",
          description: photo.description || "",
          image_url: photo.image_url || "",
          location: photo.location || "",
          camera_details: photo.camera_details || "",
          content: photo.content || "",
          meta_description: photo.meta_description || "",
          meta_keywords: photo.meta_keywords || "",
          meta_title: photo.meta_title || "",
          author: photo.author || "Rohit Pandit",
          ...
          published_at: photo.published_at 
            ? new Date(photo.published_at).toISOString().split("T")[0] 
            : new Date().toISOString().split("T")[0],
        });
  ...
  }, [imageUrl, router, encodedImageUrl]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Send PUT request to update the photo
      const response = await fetch(`/api/admin/photo/${encodedImageUrl}`, {
        method: "PUT",
        headers: {
    ...
    return (
      <div className="h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-lg">Loading photo content...</p>
        </div>
      </div>
    );
  }
