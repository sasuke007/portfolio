// Faithful TypeScript port of Inspira UI's image-trail-cursor `trail-variants.ts`
// (https://v2.inspira-ui.com/docs/en/components/cursors/image-trail-cursor).
//
// The variant classes are reproduced verbatim from the source. The only
// additions are a `destroy()` method on each class (and on `ImageItem`) plus a
// stored `rafId`/handler references — the Vue wrapper already *calls*
// `instance.destroy()`, it was simply never defined; React needs it to tear
// down the rAF loop and listeners on unmount / variant change.
...
export interface ImageTrailInstance {
  destroy(): void;
}

export const variantMap: {
  [key: string]: new (container: HTMLDivElement) => ImageTrailInstance;
} = {
