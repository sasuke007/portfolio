...
import type { Experience as Role } from "@/content";
...
export function ExperienceEntry({
  ...
  useGSAP(
    () => {
      ...
      const bullets =
        root.querySelectorAll<HTMLElement>(".entry-bullet");
    ...
    <div ref={rootRef} className="experience-entry relative">
      ...
      >
        <div className="flex items-center justify-between gap-6">
          ...
          {/* RIGHT — designation + chevron */}
          <div className="flex shrink-0 items-center gap-3">
            <span className="hidden font-body font-normal text-[clamp(0.95rem,1.1vw,1.05rem)] text-muted md:inline">
              {role.title}
            </span>
            <span
              aria-hidden
        ...
        {/* Mobile-only: designation on its own line */}
        <p className="m-0 mt-1.5 font-body font-normal text-[14px] text-muted md:hidden">
          {role.title}
        </p>

        {/* Dates + location — always shown beneath */}
        <div className="mt-1.5 flex items-center gap-2.5">
          <span className="font-body font-medium text-[12px] -tracking-display text-muted">
            {role.start}
      ...
      >
        <div className="overflow-hidden">
          <div className="pb-9 pr-2 pt-1">
            {role.summary && (
              <p className="entry-summary m-0 max-w-2xl font-display font-normal italic leading-[1.45] text-[clamp(1.05rem,1.5vw,1.3rem)] text-text opacity-0">
                &ldquo;{role.summary}&rdquo;
              </p>
            )}
